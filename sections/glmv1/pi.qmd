...
Below, I've created series of graphical representations to discern patterns, identify anomalies, and appreciate the overall behavior of the inventory data over time, establishing a baseline familiarity for the predictive analysis.
...
###### **figure 1.** *The histograms demonstrate the distribution of the dataset by variable. Figure 1.1 demonstrates the frequency of each item sold, with most items ranging between 500 and 1,300 in total. Figure 1.2 conveys the temporal dispersion of weekly sales. Both graphs exhibit a right skew, particularly pronounced in figure 1.1, implying a scarcity of high-sales items or weeks. It appears to have a poisson distribution.*
...
-   **Original**
    ...
    -   **week**: the week number.
        ...
        -   *ending* *week 53*: December 26, 2022 - January 1, 2023 <br>
-   **Feature Engineered**
...
Now it's time to determine which factors will lead to the best model using stepwise selection. It's the iterative process of adding and removing predictors based on their statistical significance. The inclusion of irrelevant variables can lead to unnecessary complexity.
...
###### **table 1.** *provides a summary of "best" models determined by several goodness-of-fit statistics. Each row represents a model configuration, specified by the model type (a or b) and the predictor variables (week, month, lag1, lag2, 4-week rolling average, best seller). The table displays R-square, the adjusted R-square, Mallows' Cp, and the residual sum of squares (RSS). An asterisk* (\*)*in the predictors' columns indicates the predictor was included in the model."*
...
The best *Cp* score is when the value is close to the total number of predictors. Table 2 shows thta model 5a has the lowest Cp score of 5.0, which is the same as the number of predictors we're using. Indicating, based on Cp, model 5a is doing the best job at fitting the data.
:::
:::
...
###### **Figure 6.** *plots the relationship between the number of predictors, ADJR2, and Cp. As expected, an the number of predictors increase the ADJR2 increases and the Cp decreases. However, the benefits appear to taper off after the inclusion of four predictors. This potentially indicates the onset of overfitting beyond this point, suggesting that a model with four predictors could strike a balance between complexity and predictive power.*

<br>

### Maximum Likelihood

**Model Selection** Given that the response variable "*sold*" represents count data and has a rightward skew, a generalized linear model (GLM) with a Poisson distribution will be deployed. To avoid overfitting, these models will be subjected to an additional level of scrutiny before selecting the best predictors. This will allow for further extrapolation and refinement
...
mod_compare %>%
 ...
 gt(rowname_col = "model_name") %>%
  tab_header(title = md("Model Comparison, part two")) %>%
  cols_align(align = "center") %>%
...
After comparing models, the predictors from 4a. will be applied to the testing set. Here's the summary of model 4a on training data:
...
-   avoids using the the highly correlated variables of week and month
-   excludes unnecessary variables, where not much is gained (see figure 6.)

## **Analytics and Evaluation**

how can match the train_preds with the training set when the model removes 0s? i want to see the predicted vs the actual
...
results <- bind_cols(predictions, is_test) %>% 
  ...
  mutate(diff = sold - .pred )  %>% 
  gather(key = "type", value = "total_sold", sold, .pred) 

#   mutate(range = ifelse(total_sold > 1000, "High", "Low"))
# 
# plot <- ggplot(results, aes(x = week, y = total_sold, color = type)) +
#   geom_point() +
#   facet_wrap(~ range, scales = "free_y") +
#   ggtitle('Weekly Count of Times Sold') +
#   xlab('Week') +
#   ylab('Count of Times Sold') +
