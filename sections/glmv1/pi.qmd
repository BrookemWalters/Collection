...
-   **Original**
    ...
    -   **week**: the week number.
        -   I asumme the following dates for the data: - *starting* *week 0*: December 27, 2021 - January 2, 2022 - *ending* *week 53*: December 26, 2022 - January 1, 2023
-   **Feature Engineered**
...
Now it's time to determine which factors will lead to the best model using stepwise selection. It's the iterative process of adding and removing predictors based on their statistical significance. The inclusion of irrelevant variables can lead to unnecessary complexity in the resulting model and can inflate the variance of prediction errors.
...
**R-squared** values explain the variability in the outcome. Based on table 2., models 3a, 4a, 4b, 5a, 5b, and 6a all have the highest R-squared values of 0.6, indicating that these models explain a significant portion (60%) of the variance in the outcome variable.
:::
:::
...
When the **RSS** is high, it means the predictions are far off from the actual values, which results in large residuals. Model 6a has the lowest RSS value of 218,998,262. This indicates that among all the models, model 6a's predictions are closest to the actual values, leading to smaller residuals.
:::
:::
...
**Model Selection** Given that the response variable "*sold*" is count data and skews right, I will also deploy a generalized linear model (GLM) with a Poisson distribution. 
To avoid over fitting, I want to take my models through one more level of scrutiny before selecting the best predictors, and extrapolate additional 
```{r}
